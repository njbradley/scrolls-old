...
void draw_image_uv(MemVecs * vecs, string texture, float x, float y, float x_scale, float y_scale, float xuv_start, float xuv_end, float yuv_start, float yuv_end) {
    float layer = 0;
    ...
    vecs->verts.insert(vecs->verts.end(), std::begin(face), std::end(face));
    vecs->uvs.insert(vecs->uvs.end(), std::begin(face_uv), std::end(face_uv));
    for (int i = 0; i < 6; i ++) {
